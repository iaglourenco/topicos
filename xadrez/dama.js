class Dama extends Peca {
    ...
    mover(tabuleiro, i, j) {
        ...
        if (
            (super.posJ == j && super.posI != i) || //anda em X
            (super.posJ != j && super.posI == i) || //anda em Y
            Math.abs(super.posJ - j) == Math.abs(super.posI - i)
        ) {
            //Andar
            if (super.posJ == j && this.trajetoria(tabuleiro, i, -1, "X", this))
                return false;
            else if (
                super.posI == i &&
                this.trajetoria(tabuleiro, j, -1, "Y", this)
            )
                return false;
            else if (
                Math.abs(super.posJ - j) == Math.abs(super.posI - i) &&
                this.trajetoria(tabuleiro, i, j, "XY", this)
            )
                return false;
    ...
    trajetoria(tabuleiro, dest1D, dest2D, coord, peca) {
        ...
        if (coord == "X") {
            ...
            if (peca.posI - dest1D > 0 && peca.posJ - dest2D > 0)
                orientacao = "NO";
            else if (peca.posI - dest1D < 0 && peca.posJ - dest2D > 0)
                orientacao = "SO";
            else if (peca.posI - dest1D > 0 && peca.posJ - dest2D < 0)
                orientacao = "NE";
            else if (peca.posI - dest1D < 0 && peca.posJ - dest2D < 0)
